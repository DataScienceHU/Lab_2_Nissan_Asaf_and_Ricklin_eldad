...
split_a <- function(a){ # a function that get function that gets string and give word list. 
  a <- str_replace_all(a,"[ \n ]", " ")
  a <- str_replace_all(a,"[^a-zA-Z ]", "") # refer to shortcut like "can't" as one word - cant.
...
by_chepter <- str_split(book, "      CHAPTER (?=[:digit:])")[[1]]
...
```{r}
get_frq <- function(string_word, string_chapters){
    chaps <- as.numeric(unlist(strsplit(string_chapters, ",")))
    output <- rep(0,length(chaps))
    for (i in 1:length(chaps)){
      a <- table( split_a(by_chepter[chaps[i]]) )
      if (string_word %in% names(a)){
        output[i] <- as.numeric(a[names(a) == string_word]) / word_n_by_chepter[chaps[i]]
        }
    }
output
}

get_frq('Ahab','1,16,2,3')
```
